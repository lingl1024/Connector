...
def run(args):
    ...
    for i in range(len(task_names)):
        ...
        if i > 0:
            ...
            p_model = copy.deepcopy(agent.model.state_dict()) # Remember knowledge of previous tasks
            ...
            agent.train_task(train_loader, val_loader)
            q_model = agent.model.state_dict() # Learn new knowledge of current task

            ans = o_model

            beta = 1.0 /  (i + 1)
            for k in o_model.keys():
                ans[k] =  p_model[k] * (1 - beta) + q_model[k] * beta # Linear connector
